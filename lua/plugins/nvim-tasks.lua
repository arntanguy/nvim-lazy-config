return {
    'Shatur/neovim-tasks',
    dependencies = { "nvim-lua/plenary.nvim", "GustavoKatel/tasks.nvim"},
    {
      "GustavoKatel/tasks.nvim",
      -- options for tasks.nvim. This will automatically call `require("tasks").setup(opts)`
      opts = {
        default_params = { -- Default module parameters with which `neovim.json` will be created.
          cmake = {
            cmd = 'cmake', -- CMake executable to use, can be changed using `:Task set_module_param cmake cmd`.
            build_dir = tostring(require('plenary.path'):new('{cwd}', 'build')), -- Build directory. The expressions `{cwd}`, `{os}` and `{build_type}` will be expanded with the corresponding text values. Could be a function that return the path to the build directory.
